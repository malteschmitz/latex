...
\begin{Frame}[fragile]{Seitenstil}{Wieviel Kopf- und Fußzeile darf es sein?}
  ...
  \begin{tabular}{r@{ }l}
    ...
    \lstinline-scrplain- & wenig Kopf- und keine Fußzeile \\
      & zum Beispiel nur Seitenzahl \\
      & (automatisch auf erster Seite eines Kapitels) \\[1ex]
    \lstinline-scrheadings- & normale Kopf- und Fußzeile \\
      & zum Beispiel Kolumnentitel \\
  ...
  \begin{Block}{Seitenstil manuell wechseln}
    \begin{lstlisting}[gobble=6,style=block]
      \pagestyle{empty} % ab aktueller Seite für alle Seiten
...
\begin{Frame}[fragile]{Seitenstil konfigurieren}{Zuordnung der Befehle zur Konfiguration der Seitenstile}
  \begin{center}
    \begin{tikzpicture}[
        node distance=1cm,
        on grid
...
\begin{Frame}[fragile]{Seitenstil konfigurieren}
  ...
  \begin{lstlisting}[gobble=4]
    \pagemark % aktulle Seitenzahl
    \leftmark % aktueller linker/gerader Kolumnentitel
    \rightmark % aktueller rechter/ungerader Kolumnentitel
    \headmark % aktueller Kolumnentitel dieser Seite
  \end{lstlisting}
  Zugriff auf aktuelle Seitenzahl und Kolumnentitel
\end{Frame}

\begin{Frame}[fragile]{Beispiele für konfigurierte Seitenstile}
  \begin{lstlisting}[gobble=4]
    \clearscrheadfoot
    \automark[section]{chapter}
  ...
  \end{lstlisting}
  \only<article>{Obige Konfigurationen erzeugen den Standard von KOMA-Script.}
...
\begin{Frame}[fragile]{Die ultimative Beispielkopfzeile}
  ...
  \includegraphics[width=10cm,page=2]{demo/kopfzeile}
\end{Frame}

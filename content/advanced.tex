...
\targets{
  ...
  \item Mit eigenen Befehlen semantische \LaTeX-Dokumente erzeugen.
  \item DIN-Briefe mit \LaTeX\ setzen können.
  \item Gestalten eigener Layouts mit \LaTeX\ kennen lernen.
}
...
\begin{Frame}[fragile]{\lstinline-scrbook-: Abschnitte langer Dokumente}
  \begin{lstlisting}[gobble=4]
    \begin{document}
...
\begin{Frame}[fragile]{Modulare Struktur}
  \begin{columns}
    ...
    \begin{tikzpicture}[%
      node distance=0,%
      grow via three points={one child at (1ex,-0.7) and
      two children at (1ex,-0.7) and (1ex,-1.4)},
      edge from parent path={([xshift=6pt]\tikzparentnode.south west) |- (\tikzchildnode.west)}]
      \tikzstyle{every node}=[thick,anchor=west,xshift=-3mm]
      \tikzstyle{every child}=[thick,draw=black]
      \tikzstyle{dir}=[draw=gray,fill=gray!30]
      \tikzstyle{optional}=[dashed]
      \node [dir] {thesis}
        child { node [dir] {content}
          child { node [maincolor] {intro.tex}}
          child { node [maincolor] (methods) {methods.tex}}
          child { node [maincolor] (conclusion) {conclusion.tex}}
        }
        child [missing] {}
        child [missing] {}
        child [missing] {}
        ...
        child [missing] {}
        child { node [dir] {preamble}
          child { node [maincolor] {styles.tex}}
        }
        child [missing] {}
        child { node [examplecolor] {main.tex}};
        \only<2>\path[alertedcolor, line width=1pt, line cap=round, shorten >=0pt,shorten <=0pt]
          (methods.east) edge (methods.west);
        \only<2>\path[alertedcolor, line width=1pt, line cap=round, shorten >=0pt,shorten <=0pt]
          (conclusion.west) edge (conclusion.east);
      \end{tikzpicture}
    \column{0.6\textwidth}
    \begin{lstlisting}[gobble=6,escapechar=-]
      \input{preamble/styles}
      \includeonly{-\alt<2>{intro}{}-}
      \begin{document}
...
\begin{Frame}[fragile]{Die Befehle}
  \begin{itemize}[]
    \item \lstinline-\input- Fügt den Inhalt der Datei an dieser Stelle ein.
    \pause
    \item \lstinline-\include- Fügt auch den Inhalt der Datei an dieser Stelle ein und Seitenumbrüche davor und dahinter.
    \item \lstinline-\includeonly- Steuert die Verwendung von \lstinline-\include-.
  \end{itemize}
\end{Frame}
